...
def main() -> None:
    ...
    for line_mail in body_mail:
        ...
        match lm_key:
            case "текст":
                add_text(paragraph, lm_value)
            case "дата":
                add_text(paragraph, release_date)
            case "текстд":
                add_text(paragraph, processing_ТекстД_template(lm_value, release_date))
            case "ссылка":
                add_hyperlink(paragraph, lm_value, lm_value)
            case "компоненты":
                add_text(paragraph, get_list_components())
            case _:
                raise MyException(
                    f'В шаблоне письма нераспознанный ключ строки - "{line_mail[0]}"',
                    777,
                )

    try:
        file_word = Path(Path.cwd(), c.WORD_NAME)
        if get_list_components() == c.END_PRINT_LIST_COMPONENTS:
            print("Нет новых компонент\nИнформационное письмо не сформировано.")
        else:
...
def add_text(paragraph, txt: str) -> None:
...
def add_hyperlink(paragraph, txt: str, url: str) -> None:
...
def get_list_components() -> str:
    """Функция get_list_components() -> str:
        Из списка файлов поддиректории NEW формирует состав выпущенных компонент.
    return:
    ...
    if sub_dir_new.is_dir():
        ...
        raise MyException(
            f"Нет доступа к каталогу {sub_dir_new}\n"
...
def processing_end_of_line(line: str) -> str:
    ...
    """
    return line[: -len(c.COMPONENT_SEPARATOR)] + c.END_PRINT_LIST_COMPONENTS
